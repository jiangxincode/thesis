...
public class InnerProduct
{
	...
	private static class InnerKey implements WritableComparable
	{
		public int i;
		public int j;
		public int k;
		...
		{
			return this.i + "\t" + this.j + "\t" +this.k + "\t" +this.identify;
		}
	}
	
	private static class InnerMap extends Mapper<IndexPair, DoubleWritable, InnerKey, DoubleWritable>
	{
		...
		public void setup(Context context)
		{
			...
			isLeftMatrix = leftMatrixPath.indexOf(conf.get("InnerProduct.InnerMap.LeftMatrixPath")) != -1? true : false;
			colNumRightMatrix = conf.getInt("InnerProduct.InnerMap.ColNumRightMatrix", 0);
			rowNumLeftMatrix = conf.getInt("InnerProduct.InnerMap.RowNumLeftMatrix", 0);
		}
	...
	private static class InnerPartitioner extends Partitioner<InnerKey, DoubleWritable>
	{
		public int getPartition(InnerKey key, DoubleWritable value, int numPartitions)
		{
			return (key.i * InnerProduct.colNumRightMatrix + key.j) % numPartitions;
		}
	}
	...
	private static class InnerReduce extends Reducer<InnerKey, DoubleWritable, IndexPair, DoubleWritable>
	{
		private IndexPair myKey = new IndexPair();
		private DoubleWritable myValue = new DoubleWritable();
		private boolean flag;
		private double sum, tmp;
		public void reduce(InnerKey key, Iterable<DoubleWritable> values, Context context) throws IOException, InterruptedException
		{
			...
			//System.out.println(myKey.x + "\t" + myKey.y + " : " + myValue.toString());
		}
	}
	
	private static void runInnerProduct(int rowNumLeftMatrix, int colNumLeftMatrix, int rowNumRightMatrix, int colNumRightMatrix, String leftMatrixPath,
			String rightMatrixPath, String outputDir, int numReduce) throws Exception
	{
		...
		conf.setInt("InnerProduct.InnerMap.ColNumRightMatrix", colNumRightMatrix);
		conf.setInt("InnerProduct.InnerMap.RowNumLeftMatrix", rowNumLeftMatrix);
		System.out.println(conf.get("InnerProduct.InnerMap.LeftMatrixPath") + "\t" + leftMatrixPath);
		...
		FileInputFormat.addInputPath(innerProductJob, new Path(rightMatrixPath));
	...
	public static void main(String[] args) throws Exception
	{
		...
		runInnerProduct(rowNumLeftMatrix, colNumLeftMatrix, rowNumRightMatrix, colNumRightMatrix, leftMatrixPath, rightMatrixPath, outputDir, numReduce);
